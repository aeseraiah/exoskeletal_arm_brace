  ...
  while (1){
    ...
    if (elapsedTime >= switchDuration) {
      ...
      if (currentLabel == "flexion") {
        currentLabel = "extension";
        state = 1;
      } else if (currentLabel == "half flexion"){
        // labelStartTime = millis();
        currentLabel = "extension";
        state = 0;
      } else if (currentLabel == "extension" && state == 1){
        currentLabel = "half flexion";
      } else {
        currentLabel = "flexion";
        labelStartTime = millis();
      }
    ...
    trisumOfSquares = 0;
    initial = micros();
    //gather 250 samples, or .5 seconds, of data from each sensor
    for (samples = 0; samples<250; samples ++){
      start = micros();
      biVal = readBi();
      //set biBuffer value to biVal if it exceeds threshold for activation, otherwise = 0
      biBuffer[samples] = (biVal > biThresh) ? biVal : 0;
      Serial.print(biBuffer[samples]);
      // bisumOfSquares += sq(biBuffer[samples]);
      delayMicroseconds(50);
      triVal = readTri();
      //set triBuffer value to triVal if it exceeds threshold for activation, otherwise = 0
      triBuffer[samples] = (triVal > triThresh) ? triVal : 0;
      Serial.print(", ");
      Serial.println(triBuffer[samples]);
      // trisumOfSquares += sq(triBuffer[samples]);
      end = micros();
      //ensures sampling at 2000 microsecond intervals (500 Hz)
      delayMicroseconds(timeBudget - (end-start));
    }
    // time = micros() - initial;
