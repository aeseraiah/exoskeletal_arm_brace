...
float model_training_and_predictions(EMGData data[32], unsigned int number_data_points, train_model_param) {
  while (!Serial);
...
float make_predictions(EMGData_for_predictions data[32], unsigned int number_data_points, aimodel_t trained_model) {
  ...
  uint32_t memory_size = aialgo_sizeof_inference_memory(&trained_model);
  ...
  // Schedule the memory over the model
  aialgo_schedule_inference_memory(&trained_model, memory_ptr, memory_size);
...
void loop() {
  ...
  while (1){
    ...
    // only store the first number_data_points RMS values (2 seconds of data)
    if (emgIndex < 32) {
    ...
    if (elapsedtime >= labelDuration_model_training) {
      if (emgIndex == num_data_points) {
        ...
        float accuracy = model_training_and_predictions(emg_trainingData, num_data_points);
        
        if model accuracy is above 85%, reak out of loop and continue to actuation of servo:
        if (accuracy > 40) { // change to 85 later
          // Serial.println("Model accuracy is above 85%. Continue to make_predictions function. Then to actuation.");
          Serial.println("Continue to make_predictions function. Then to actuation.");
