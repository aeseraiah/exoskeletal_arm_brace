...
EMGData emgData[num_data_points]; // stores real-time emg data used to train model
EMGData_for_predictions emgData[num_data_points] // stores real-time emg data used to make predictions
...
float model_training(EMGData data[32], unsigned int number_data_points) {
  while (!Serial);
...
float make_prediction(EMGData data[32], unsigned int number_data_points) {
  while (!Serial);
  //AIfES requires random weights for training
  //Here the random seed is generated by the noise of an analog pin
  srand(analogRead(A5));
...
void loop() {
  ...
  while (1){
    ...
    if (emgIndex < 32) {
        emgData[emgIndex].biRMS = biRMS;
        emgData[emgIndex].triRMS = triRMS;
        emgData[emgIndex].label = currentLabel;
