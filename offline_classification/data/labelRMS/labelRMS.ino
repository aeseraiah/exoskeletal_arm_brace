...
float model_training(double bicepRMS, double tricepRMS) {
  while (!Serial);
  ...
  uint16_t output_shape[] = {16, 1};
  float output_data[16*1];
...
void loop() {
  ...
  unsigned long labelDuration_model_training = 20000; // collect and label data for 20 seconds, then start model training
